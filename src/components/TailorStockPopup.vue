...
<v-dialog v-model="dialog" max-width="500">
    ...
    <v-card title="Stock Settings">
        <v-card-text>
            <p class="pb-4">
                You can exclude a stock entirely with the below toggle, or scale the weighting of the stock in the index by some percent. The default 100% would not change the weight in the index.
            </p>
            <v-text-field v-model="ticker" label="Ticker" 
            color="blue-darken-4" density="compact" hide-details variant="solo" inline inset></v-text-field>
            <v-divider />
            <v-switch v-model="excluded" :label="excluded ? 'Exclude' : 'Keep'" 
            color="blue-darken-4" density="compact" hide-details inline inset></v-switch>

            <v-text-field v-model="weight" :disabled="excluded" label="Weighting %" 
            variant="solo" color="blue-darken-4" density="compact" :rules="numberValidationRules" 
            hide-details inline inset></v-text-field>
        ...
        <v-card-actions class="justify-center px-6 py-3">
            ...
            <v-btn :disabled = "!hasValidChanges" class="text-white flex-grow-1 text-none" 
