  ...
  <v-card class="mx-auto" max-width="344" title="Login to Provider">
    <v-form v-model="form" @submit.prevent="login">
      <v-container>
        ...
        <v-text-field :readonly="loading" :rules="[required]" v-model="secret" 
        color="primary" label="API Secret"
      ...
      return axios.post('http://localhost:3000/login',
        { 'key': this.key, 'secret': this.secret, 'provider': this.selectedProvider}).then(() => {
          ...
          if (this.saveCredentials) {
            this.storeSavedValue({'key':'key-'.concat(this.selectedProvider), 'value':this.key});
            this.storeSavedValue({'key':'secret-'.concat(this.selectedProvider), 'value':this.secret});
          }
          this.loading = false;
