<template>
  <v-card>
    <v-card-title class="d-flex justify-center align-center"
      >Configure {{ portfolioName }}</v-card-title
    >
      ...
      <v-col cols="6" min-width="300px">
        <v-card>
          <v-card-title style="min-height: 90px" class="text-center">
            <v-select
              v-model="selectedSubPortfolio"
              :items="subPortfolioKeys"
              density="compact"
              variant="solo"
              label="Sub Portfolio"
            ...
            <v-progress-linear
              ...
              v-model="portfolioPercentOfGoal"
              height="15"
            >
              <span class="text-caption"
                >{{ portfolioPercentOfGoal }}% of Target Size</span
              >
            </v-progress-linear>
            <v-progress-linear
            ...
            >
              <span class="text-caption"
                >{{ portfolioPercentOfTarget }}% Only Index Stocks</span
              >
            </v-progress-linear>
          </v-card-title>
          <v-card-actions>
    ...
    <v-card-actions class="pa-6">
      <v-row>
        <v-col>
          <v-row v-if="targetPortfolio" justify="center"
            >Showing up to {{ displayLength }} of
            {{ portfolio.holdings.length }} and
            {{ targetPortfolio.holdings.length }}</v-row
          >
          <v-row v-else justify="center"
            >Showing up to {{ displayLength }} of
            {{ portfolio.holdings.length }}</v-row
          >
          <v-row justify="center"
            ><v-btn v-if="maxLength > displayLength" @click="expandList()"
              >Load More</v-btn
            ></v-row
          >
        </v-col>
  ...
  computed: {
    ...
    displayPortfolio() {
      ...
      if (this.selectedSubPortfolio === "All") {
        ...
        return this.portfolio.components.find(
          (portfolio) => portfolio.provider === local.selectedSubPortfolio,
        );
      }
    },
    ...
    portfolio() {
      const matched = this.compositePortfolios.find(
        (portfolio) => portfolio.name === this.portfolioName,
      );
    ...
    portfolioPercentOfTarget() {
      return Math.round(
        (this.portfolio.valueInIndex(this.targetPortfolio) /
  ...
  methods: {
    ...
    handlePortfolioSizeInput: debounce(function () {
      ...
      this.setPortfolioSize({
        ...
        size: Number(this.portfolioTarget),
      });
    ...
    getTargetPortfolio(newValue) {
      ...
      return instance["makeAsyncRequest"]("generate_index", {
        ...
        index: target,
        stock_exclusions: Array.from(
          this.portfolioCustomization.excludedTickers,
        ),
        ...
        .then((response) => {
          ...
          this.setTargetPortfolio({
            portfolioName: this.portfolioName,
            targetPortfolio: targetPortfolio,
          });
          if (newValue) {
            this.setPortfolioTargetIndex({
              portfolioName: this.portfolioName,
              index: target,
            });
          }
    ...
    updatedPortfolioWithTargets() {
      ...
      this.portfolio.holdings.forEach((element) => {
        const targetElement = this.targetPortfolio.holdings.find(
          (target) => target.ticker === element.ticker,
        );
        if (targetElement) {
    ...
    expandList() {
      let next = Math.min(
        this.maxLength,
        this.$store.getters.displayLength + 50,
      );
