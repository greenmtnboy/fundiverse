<template>
    <v-card>
      <v-card-title class="d-flex justify-center align-center">Default Portfolio Management</v-card-title>
  ...
  export default {
    ...
    methods: {
      ...
      getPortfolio() {
        ...
        return instance.get(`http://localhost:3000/composite_portfolio/default`).then((response) => {
          const portfolioHoldings = response.data.holdings.map(
