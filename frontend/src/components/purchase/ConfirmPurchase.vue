    ...
    <v-card :title="`Submit Purchase`">
      ...
      <v-card-text>
        <p>
          ...
          If you do not have enough money to buy the entire portfolio, you can
          prioritize which tickers to buy first by selecting a purchase order.
        </p>
      </v-card-text>
      <v-card-text>
        <p>
          <b>Smallest diff first</b> will prioritize stocks where you can get
          the full target value with a smaller order, while
          <b>largest diff</b> will prioritize the stocks with the largest delta
          from target value. <b>Even spread</b> will proportionally spread your
          purchase order over all target stocks, and may result in a very large
          number of orders.
        </p>
        ...
        <v-row v-if="localExclusions.length > 0">
          <v-col width="12" justify="center">
            Excluded Tickers
...
export default {
  ...
  methods: {
    ...
    planPurchase() {
      ...
      return instance
        .post("plan_composite_purchase", {
          to_purchase: this.toPurchase,
          target_size: this.targetSize,
          index: this.selectedIndex,
          stock_exclusions: Array.from(
            this.customizations.excludedTickers,
          ).concat(this.localExclusions),
    ...
    submit() {
      ...
      this.placedPage = 1;
      return instance
        .post("buy_index_from_plan_multi_provider", {
          plan: this.plan,
          providers: this.providers,
        })
        .then((resp) => {
