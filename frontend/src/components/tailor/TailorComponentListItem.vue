  ...
  <v-list-item>
    <v-row>
      <v-col cols="3" class="d-flex align-center">
        <v-list-item-title>
          <v-btn density="compact" @click="openGithub" icon>
            <v-icon>mdi-github</v-icon> </v-btn
          ><span class="px-1">{{ list }}</span>
          <v-chip
            :color="weight >= 1 ? 'green' : 'red'"
            v-if="weight"
            small
            outlined
            >{{ weight * 100 }}%</v-chip
      ...
      <v-col cols="9">
        <v-chip-group class="d-flex align-center" active-class="primary">
          ...
          <v-chip v-if="tags.length > displayedTags.length" label> ... </v-chip>
    ...
    <template v-slot:append>
      <v-tooltip>
        <template v-slot:activator="{ props }">
          <v-btn
            v-bind="props"
            @click="(_) => removeListExclusion(list)"
            icon
            density="compact"
          >
