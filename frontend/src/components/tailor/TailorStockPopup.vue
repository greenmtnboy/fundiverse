    ...
    <v-card title="Stock Settings">
      <v-card-text>
        ...
        <v-text-field
          ...
          inline
          inset
          @update:modelValue="handleTickerInputUpdate"
        ></v-text-field>
        <v-banner lines="two" icon="$warning" color="primary">
          ...
          <v-banner-text v-else-if="updateType == 'reweight'">
            {{ ticker }} is already in the index, you can exclude it or reweight
            the current % by a new %. (100% would mantain currrent weight.)
          </v-banner-text>
          <v-banner-text v-else>
            {{ ticker }} is not in the index, you can add it with a base
            {{ weight }}% target. (1% would be 1% of your target index.)
          </v-banner-text>
        </v-banner>
        <v-switch
          v-if="updateType == 'reweight'"
          v-model="excluded"
          class="pb-2"
          :label="
            excluded
        ...
        <v-text-field
          v-model="weight"
          :disabled="excluded"
          :label="updateType == 'reweight' ? 'Reweight %' : 'Target %'"
          variant="solo"
          color="blue-darken-4"
          density="compact"
