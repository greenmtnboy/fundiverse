  ...
  <v-dialog v-model="visible" fullscreen hide-overlay :scrim="false">
    ...
    <v-card>
      <v-toolbar dark color="primary">
        ...
        <!-- </v-btn> -->
        <v-toolbar-title
          >Customizations for {{ portfolioName }}</v-toolbar-title
        >
        <v-spacer></v-spacer>
        <v-toolbar-items>
      ...
      <v-card-text>
        ...
        <v-card rounded="0">
          ...
          <v-card-text>
            <template
              v-for="element in stockModifications"
              :key="element.ticker"
            >
              <TailorStockListItem
                mode="modification"
...
export default {
  ...
  computed: {
    ...
    customizations() {
      const customizations = this.portfolioCustomizations.get(
        this.portfolioName,
      );
      if (customizations) {
  ...
  methods: {
    ...mapActions([
      ...
      "removeStockExclusion",
      "removeStockModification",
      "removeListExclusion",
    ...
    saveDefaultModificationsLocal() {
      this.isSaving = true;
      setTimeout(() => {
        this.saveDefaultModifications({ portfolioName: this.portfolioName })
          .then(() => {
    ...
    clearAllModificationsLocal() {
      ...
      this.clearAllModifications({ portfolioName: this.portfolioName })
        .then(() => {
